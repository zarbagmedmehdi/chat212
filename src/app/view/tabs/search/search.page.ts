...
import {AlertController, IonLabel, IonSelect} from '@ionic/angular';
...
prenom:string="";
bb:number=0;
  constructor( private searchService :SearchService,private alert: AlertController) {
  ...
  onSelect() {
   ...
   async paysSearch() {
    const alert = await this.alert.create({
      ...
      buttons: [{text: 'Cancel', role: 'cancel', cssClass: 'secondary', handler: () => {return null}},
  ...
  async textSearch(header:string,variable) {
    const alert = await this.alert.create({
      ...
      buttons: [
        {text: 'Cancel', role: 'cancel', cssClass: 'secondary', handler: () => {return null}},
        {text: 'Ok', handler: datas => {
